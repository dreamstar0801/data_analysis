/* ============================================================================
   ...
   Source  : data_analysis/gamezone-orders-data.xlsx  (21,864 raw order rows,
             2019-01 through 2021-12, 151 country codes -- a default pandas
             read reports 150, because Namibia's ISO code is the string 'NA'
             and gets coerced to null. Same defect as the region bug below.)
   ...
   Contents:
       ...
       Q7      Region mapping guard (fails loudly if the 'NA' defect returns)
   ========================================================================== */
...
/* ============================================================================
   ...
   nullstr = '' is not decoration. DuckDB's CSV reader treats an empty field
   as NULL and leaves other text alone, but stating it explicitly documents
   the contract these files depend on: the ONLY null token is the empty
   string. 'NA' is data here -- North America in the region column, Namibia
   in the country column -- and must survive the load as literal text.
   The CSVs themselves are regenerated by scripts/export_workbook_to_csv.py,
   which reads the workbook with keep_default_na=False for the same reason.
   ========================================================================== */

CREATE OR REPLACE VIEW stg_orders AS
SELECT *
FROM read_csv('data/orders.csv', header = true, all_varchar = true, nullstr = '');

CREATE OR REPLACE VIEW raw_region AS
SELECT *
FROM read_csv('data/region_lookup.csv', header = true, all_varchar = true, nullstr = '');


/* ----------------------------------------------------------------------------
   Region staging.
   The lookup is complete: US and CA are both present and both map to North
   America. The problem is the value they map to.
---------------------------------------------------------------------------- */
CREATE OR REPLACE VIEW stg_region AS
SELECT
    country_code,
    CASE
        -- 'NA' is the source's code for North America and is also the default
        -- null token in pandas and most CSV loaders. Renaming to 'NAMER' here
        -- makes the collision structurally impossible downstream. Parsing
        -- carefully is a fix that depends on every future reader remembering
        -- to parse carefully; eliminating the token does not. Do not revert.
        WHEN UPPER(TRIM(region_cleaned)) IN ('NA', 'NORTH AMERICA') THEN 'NAMER'
        ELSE UPPER(TRIM(region_cleaned))
    END AS region
FROM raw_region;
...
/* ============================================================================
   ...
   Mirrors the cleaning applied in the Excel workbook, stated as code:
     ...
     4. purchase_ts arrives in two formats (ISO, and 10 rows of MM-DD-YYYY,
        ...
        1 further row is whitespace-only and stays null
...
SELECT
    ...
    /* No analyst patch. The source lookup maps every country in this dataset
       except the two non-ISO codes AP and EU; 'Unmapped' now means genuinely
       unmapped (42 orders, $3,527) rather than "lost on load". */
    COALESCE(NULLIF(r.region, ''), 'Unmapped')         AS region
FROM typed t
LEFT JOIN stg_region r ON r.country_code = t.country_code;
...
FROM (
    ...
    SELECT 9,
           ...
           (SELECT COUNT(*) FROM orders_clean c
             WHERE c.country_code IS NOT NULL
               AND NOT EXISTS (SELECT 1 FROM stg_region
                                WHERE country_code = c.country_code)),
           'AP and EU: region abbreviations leaked into a country column'
    UNION ALL
    SELECT 10,
           'orders in Unmapped region',
           (SELECT COUNT(*) FROM orders_revenue WHERE region = 'Unmapped'),
           '37 blank country codes + 5 orders coded AP/EU'
    UNION ALL
    SELECT 11,
    ...
    UNION ALL
    SELECT 12,
           'distinct country codes in orders',
           (SELECT COUNT(DISTINCT country_code) FROM orders_clean),
           '151; a default pandas read reports 150 (Namibia = NA)'
) AS checks
ORDER BY ord;


/* ============================================================================
   Q7 - Region mapping guard
   This is the regression test for the defect that produced the wrong regional
   story in the first place: North America's source region code is the literal
   string 'NA', which pandas and most CSV loaders read as null. If that ever
   happens again, NAMER revenue silently drains into a null bucket and EMEA
   looks like the largest region.

   Four assertions, reported as a readable PASS/FAIL table rather than a bare
   error, so a failing run says what it observed and what it expected.
   Anything other than four PASS rows means STOP: the numbers above are wrong.

   Checks 1-3 catch a reader that re-enables null coercion (NAMER drains away).
   Check 4 catches the opposite failure: someone deletes the rename in
   stg_region as redundant, and the raw 'NA' token flows downstream again.
   Verified by deliberately re-exporting the CSVs with pandas defaults:
   checks 1, 2 and 3 flip to FAIL.
   ========================================================================== */
WITH totals AS (
    SELECT SUM(usd_price) AS revenue_all FROM orders_revenue
),
namer AS (
    SELECT SUM(usd_price) AS revenue_namer
    FROM orders_revenue WHERE region = 'NAMER'
)
SELECT 'Q7 region mapping guard' AS query_name, check_name, observed, expected,
       CASE WHEN passed THEN 'PASS' ELSE 'FAIL' END AS result
FROM (
    SELECT 1 AS ord,
           'distinct mapped regions' AS check_name,
           CAST((SELECT COUNT(DISTINCT region) FROM orders_revenue
                  WHERE region <> 'Unmapped') AS VARCHAR) AS observed,
           '4 (NAMER, EMEA, APAC, LATAM)'                 AS expected,
           (SELECT COUNT(DISTINCT region) FROM orders_revenue
             WHERE region <> 'Unmapped') = 4              AS passed
    UNION ALL
    SELECT 2,
           'unmapped revenue-eligible orders',
           CAST((SELECT COUNT(*) FROM orders_revenue
                  WHERE region = 'Unmapped') AS VARCHAR),
           '42 (37 blank country_code + 5 coded AP/EU)',
           (SELECT COUNT(*) FROM orders_revenue WHERE region = 'Unmapped') = 42
    UNION ALL
    SELECT 3,
           'NAMER share of revenue',
           (SELECT COALESCE(
                       CAST(ROUND(100.0 * revenue_namer / revenue_all, 1) AS VARCHAR) || '%',
                       'no NAMER rows at all')
              FROM namer, totals),
           '52.2% +/- 0.5pp',
           (SELECT COALESCE(ABS(100.0 * revenue_namer / revenue_all - 52.2) <= 0.5, FALSE)
              FROM namer, totals)
    UNION ALL
    SELECT 4,
           'literal ''NA'' surviving into staged regions',
           CAST((SELECT COUNT(*) FROM stg_region WHERE region = 'NA') AS VARCHAR),
           '0 (all renamed to NAMER)',
           (SELECT COUNT(*) FROM stg_region WHERE region = 'NA') = 0
) AS guard
ORDER BY ord;
